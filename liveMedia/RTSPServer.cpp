...
char* RTSPServer::rtspURLPrefix(int clientSocket) const
{
	...
	if (clientSocket < 0) {
		// Use our default IP address in the URL:使用我们的默认IP地址的URL
...
{
	// 创建一个流式socket，绑定端口httpPort和INADDR_ANY
...
int RTSPServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort)
{
	...
	do {
		...
		//下面的语句是默认启用的。
		//不要禁用它（通过定义ALLOW_RTSP_SERVER_PORT_REUSE允许RTSP信息服务器端口重用），除非你知道自己在做什么。
#ifndef ALLOW_RTSP_SERVER_PORT_REUSE
		// NoReuse构造的时候设置env的groupsockPriv->reuseFlag=0
		NoReuse dummy(env); // Don't use this socket if there's already a local server using it如果有已经在使用它的本地服务器,不要使用这个套接字
#endif
		// 创建一个流式套接字
		ourSocket = setupStreamSocket(env, ourPort);
		if (ourSocket < 0) break;

		// Make sure we have a big send buffer:确保我们有一个大的发送缓冲区：
		if (!increaseSendBufferTo(env, ourSocket, 50 * 1024)) break;

		// Allow multiple simultaneous connections:允许多个并发连接：
...
{
#ifdef USE_SIGNALS
	// Ignore the SIGPIPE signal, so that clients on the same host that are killed忽略SIGPIPE信号，从而使在同一主机上的客户端被杀死
	// don't also kill us:没有同时杀死我们
	signal(SIGPIPE, SIG_IGN);
#endif

	// Arrange to handle connections from others:安排专人处理来自其他连接：
...
{
	// Turn off background read handling:关闭后台处理读
	...
	// Remove all server media sessions (they'll get deleted when they're finished):删除所有服务器媒体会话（当他们完成后，他们将被删除）：
	while (1) {
	...
	// Finally, delete the session table itself:最后，删除会话表本身：
...
void RTSPServer::incomingConnectionHandler(int serverSocket)
{
	...
	// Create a new object for this RTSP session.创建一个新的RTSP会话对象
	...
	// （选择一个随机的32位整数用于会话ID（它会被编码为8位十六进制数），我们也懒得检查碰撞;两个并发会话得到相同的会话ID的概率是非常低的。）
