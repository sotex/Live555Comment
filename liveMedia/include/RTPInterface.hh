...
public:
	tcpStreamRecord* fNext;	//下一个TCP流记录
	int fStreamSocketNum;	//流socket号
	unsigned char fStreamChannelId;	//流通道ID
};
...
public:
	...
	{ return fNextTCPReadStreamSocketNum; }
	unsigned char nextTCPReadStreamChannelId() const 
	{ return fNextTCPReadStreamChannelId; }

private:
	friend class SocketDescriptor;
	Medium* fOwner;		//绑定媒体对象
	Groupsock* fGS;		//组Socket对象
	tcpStreamRecord* fTCPStreams; // 可选的optional, for RTP-over-TCP streaming/receiving

	unsigned short fNextTCPReadSize;
	// how much data (if any) is available to be read from the TCP stream
	// 有多少数据（如果有的话）可供可以从TCP流中读取
