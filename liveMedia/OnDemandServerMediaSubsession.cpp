...
fSDPLines(NULL), fReuseFirstSource(reuseFirstSource), fInitialPortNum(initialPortNum), fLastStreamToken(NULL)
{
	...
	fCNAME[sizeof fCNAME - 1] = '\0'; // just in case 以防万一
}
...
OnDemandServerMediaSubsession::~OnDemandServerMediaSubsession()
{
	...
	// Clean out the destinations hash table:清理掉目标地址哈希表
	while (1) {
...
OnDemandServerMediaSubsession::sdpLines()
{
	if (fSDPLines == NULL) {
		...
		// 我们需要建立一组描述这个子会话（如单播流）SDP行。要做到这一点，
		// 我们首先创建虚拟（未使用）源和“RTPSink”的对象，我们使用SDP行的参数：

		unsigned estBitrate;	//est比特率
		FramedSource* inputSource = createNewStreamSource(0, estBitrate);//创建流源
		if (inputSource == NULL) return NULL; // file not found

		struct in_addr dummyAddr;	//伪地址
		dummyAddr.s_addr = 0;
		Groupsock dummyGroupsock(envir(), dummyAddr, 0, 0);	// 伪组套接字
		unsigned char rtpPayloadType = 96 + trackNumber() - 1; // if dynamic
		RTPSink* dummyRTPSink	/*伪RTP槽*/
			= createNewRTPSink(&dummyGroupsock, rtpPayloadType, inputSource);
		// 设置SDP行
		setSDPLinesFromRTPSink(dummyRTPSink, inputSource, estBitrate);
		Medium::close(dummyRTPSink);	//关闭伪RTP槽
		closeStreamSource(inputSource);	//关闭流源
	}
...
StreamState::StreamState(OnDemandServerMediaSubsession& master,
	...
	fTotalBW(totalBW), fRTCPInstance(NULL) /* created later 以后创建*/,
