...
#ifdef USE_SYSTEM_RANDOM	//如果定义了使用系统随机函数宏
 ...
 */
#define	TYPE_0		0		/* linear congruential线性同余 */
#define	BREAK_0		8
#define	DEG_0		0
#define	SEP_0		0
 ...
 */
static long* fptr = &randtbl[SEP_3 + 1];	//front pointer
static long* rptr = &randtbl[1];			//rear pointer
...
our_srandom(unsigned int x)
{
	...
	if (rand_type == TYPE_0)
		...
		for (i = 1; i < rand_deg; i++)
			state[i] = 1103515245 * state[i - 1] + 12345;	//线性同余算法
		fptr = &state[rand_sep];
		rptr = &state[0];
